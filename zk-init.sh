...
then
    ...
    echo "server.$MYID=$IPADDRESS:2888:3888:observer;$IPADDRESS:2181" >> /opt/zookeeper/conf/zoo.cfg.dynamic
    ...
    /opt/zookeeper/bin/zkCli.sh -server $ZK:2181 reconfig -add "server.$MYID=$IPADDRESS:2888:3888:participant;$IPADDRESS:2181"
    /opt/zookeeper/bin/zkServer.sh stop
    ZOO_LOG_DIR=/var/log ZOO_LOG4J_PROP='INFO,CONSOLE,ROLLINGFILE' /opt/zookeeper/bin/zkServer.sh start-foreground
else
    # start a new cluster
    echo "server.$MYID=$IPADDRESS:2888:3888;$IPADDRESS:2181" >> /opt/zookeeper/conf/zoo.cfg.dynamic
    /opt/zookeeper/bin/zkServer-initialize.sh --force --myid=$MYID
    ZOO_LOG_DIR=/var/log ZOO_LOG4J_PROP='INFO,CONSOLE,ROLLINGFILE' /opt/zookeeper/bin/zkServer.sh start-foreground
fi
